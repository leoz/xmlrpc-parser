...
XMLRPCMessage.prototype.xml = function() {
  ...
  var ele = this.method
    ? xmlBuilder.create("methodCall").ele("methodName", this.method)
...
XMLRPCMessage.prototype.getParamXML = function(ele, data) {
  ...
  switch (type) {
    case "date":
      xml = this.toDateXML(ele, data);
      break;
    case "array":
      xml = this.toArrayXML(ele, data);
      break;
    case "struct":
      xml = this.toStructXML(ele, data);
      break;
    case "boolean":
      xml = this.toBooleanXML(ele, data);
      break;
    default:
      xml = this.toValueXML(ele, type, data);
      break;
  }
  return xml;
};
